...
export default function BenefitsCardV1({item,data,duplicate=false}:{item:any;data:any;duplicate?:boolean}){
 const href=safeHref(item?.url), blank=String(item?.linkTarget||'new')==='new'
 const media=item?.mediaType==='image'&&item?.image?.asset
  ?<Image className="bhx-benefit-image" src={urlFor(item.image).width(320).height(320).url()} alt={duplicate?'':clean(item.title)} width={160} height={160}/>
 ...
 if(duplicate)return <div className="bhx-benefit-card" aria-hidden="true" data-bhx-marquee-clone="true">{body}</div>
